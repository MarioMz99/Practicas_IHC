...
namespace IBM.Watsson.Examples
{
    ...
    {
        public delegate void OnTranscriptRecevied(string transcript);
        public OnTranscriptRecevied onTranscriptRecevied;
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        //Validar un posible comando recibido
                        if (res.final)
                        {
                            onTranscriptRecevied?.Invoke(alt.transcript);
                        }
                    }
